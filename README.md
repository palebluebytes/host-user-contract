...
- [`examples/users/`](examples/users/) — the reference **user fleet** (ADR-0020): the operator's
  ...
  for the minimal `sudo`/wheel grant, login password `password`), and the `duo-a`/`duo-b` pair, which
  import one shared home module + overlay from `examples/users/shared/` — the ADR-0020 "shared code,
  per-user data" ergonomic, which is permitted, not required (the five above share nothing).
