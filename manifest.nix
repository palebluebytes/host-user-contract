...
{ lib }:
let
  # The current manifest version. v2 added `granted` (the enabled feature names baked into the
  # home — the ADR-0016 coupling-guard field) to v1's `{ version, username, requests, packages }`.
  # `writeManifest` emits this by default; `readManifest` accepts EITHER (a v1 manifest predates
  # `granted`, so it reads back as `[ ]` — the v1→v2 compat).
  currentVersion = 2;
  ...
  writeManifest =
    ...
    builtins.toFile "contract-requests-${username}.json" (
      builtins.toJSON (
        ...
        # `granted` exists only from v2 on — a v1 manifest omits it (the read side defaults it to []).
        // lib.optionalAttrs (version >= 2) { inherit granted; }
      )
    );

  # readManifest: parse a pinned manifest file (`${contractPackage}/${manifestFileName}`, already in
  # the store) into the canonical field set. Applies the v1→v2 compat read: `granted` (absent in v1)
  # normalizes to `[ ]`, and `packages` (defensively) too. A plain `importJSON` — the file is
  # pre-built and pinned (a realized store path), so this is not IFD.
  readManifest =
    manifestFile:
    let
