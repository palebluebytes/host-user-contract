...
# ROUND-TRIP (`readManifest (writeManifest fields)` yields the original account fields, incl. a v1
# manifest read with the grant-key field absent), and an equivalence check that each repo fixture's JSON is
# byte-identical to `writeManifest`'s output — so the fixtures are GENERATED by the schema owner and
# cannot silently drift from it, while staying realized repo paths (no IFD).
{
  pkgs,
  toolkit,
  mkContractPackage,
...
let
  ...
  # grants must match the direct call fed the same grants — so the baked grant-key feature names
  ...
  # `gui` failure inverted, and equally undetectable downstream (an empty manifest grant-key is
  ...
  # `referenceFields` is v1 (no grant-key, exercising the v1->v2 compat); `guiFields` is v2.
  referenceFields = {
    version = 1;
    username = "ada";
    requests = {
  ...
  # v1 compat: a v1 manifest (written with no grant-key) reads back with `grantKey = [ ]`.
...
in
{
  assertions = [
    ...
    # mkContractPackageForHome: grants forward identically — the baked grant-key names survive.
    {
    ...
    {
      name = "manifest round-trip: writeManifest stamps the current version (2)";
      ok = guiRoundTrip.version == 2;
    }
    # manifest v1->v2 compat: a v1 manifest (field absent) reads back with grantKey = []
    {
      name = "manifest compat: a v1 manifest reads back with grantKey = [] (v1->v2 compat)";
      ok =
        referenceRoundTrip.version == 1
        && referenceRoundTrip.grantKey == [ ]
        && builtins.removeAttrs referenceRoundTrip [ "grantKey" ] == referenceFields;
    }
