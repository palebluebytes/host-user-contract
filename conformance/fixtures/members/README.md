...
A synthetic ADR-0020 users directory for `conformance/members.nix`, holding the three kinds of
...
- `half-added/` — a directory with **no `identity.json`** (a user whose `home.nix` landed first).
  Not a member: the member set keys on the identity file, so a half-added directory is skipped rather
...
empty-member-set error is proven against — pointed at directly, it derives no member at all.
