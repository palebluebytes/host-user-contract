...
**Status:** Accepted as the model; **not yet implemented** here. **Amended by [ADR-0016](0016-prebuilt-binding-mode.md)**: the `execPayload` registry flag and its exclusion from `runtimeEligibleFeature` are deferred — no feature carries a host-executed payload yet; the mechanism re-enters when a concrete feature requires it. The `contract.requests` channel this ADR introduces does not exist in the contract yet — feature configuration still rides the system side (`custom.users.<u>.<feature>`, per [ADR-0003](0003-feature-configuration-aggregates.md)), which is the staged starting point this ADR's "known knots" #2 sanctions; the home-side requests namespace is tracked by issue #5. The three in-tree leaks this ADR closes are *fleet-repo* artifacts (this contract repo ships no user module). **Supersedes** [ADR-0001](0001-host-user-contract.md) mechanic 7's curated-catalog model (replaced by the `contract.requests` channel) and the slice-09 "model C" meta-issue. The runtime-binding half is carried forward by [ADR-0006](0006-anyhost-greeter-runtime-binding.md) and [ADR-0007](0007-user-flake-shape.md). **The ignore-overreach half of the enforcement pair below is superseded by [ADR-0028](0028-user-voice-is-typed-and-lives-in-the-home.md)**: `contract.requests` carries no freeform, so an *unknown* feature key now errors exactly as a *malformed known* one does — validate-intent everywhere; tolerance survives only in `traceUser`'s permissive inspector mode. [ADR-0028](0028-user-voice-is-typed-and-lives-in-the-home.md) also **formalises the offer** this ADR left *implicit* ("no formal `offers` field until the separate-repo future needs one") as the typed, home-side `contract.wants`. **Clarified by issue #59**: "the grant is the sole enabler; degradation is silent" below is about the **host's** veto and only that — a request the host did not grant stays inert, unchanged. Once ADR-0028 gave the user its own typed veto, a second case appeared that this ADR never spoke to: a request for a feature the **user itself** set `contract.wants.<f>.enable = false` on. Since the grant is `affordances ∩ offer`, no host can ever rescue it, so it is dead data rather than a degradation and `mkContractUser` fails the bake on it. **Confirmed by [ADR-0031](0031-published-bake-set-unclassified-unpruned.md)** (2026-08-18): model C was re-proposed as an *automatic* gui/CLI classifier over home-manager's option namespaces and re-tested empirically — the candidate signals give **zero** discrimination and the catalog would run to 592 leaf namespaces churning on every home-manager bump, so the rejection recorded here now stands on measurement as well as on principle. **Narrowed by [ADR-0032](0032-grants-ride-the-bind-modes-build-homes.md) (2026-08-18)**: the degradation posture still governs GRANTS (an ungranted feature stays inert), but a MODE mismatch is a refusal — a user supporting only `gui` cannot be bound by a host that runs only `cli`, and that is a hard error rather than a lesser home.
