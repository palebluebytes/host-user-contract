...
# prints.
#
# It builds a small `contract-account-plan <identity.json> <grants.json>` tool that evaluates the
...
# evaluator's record equals `accountPlan`'s.
{
  ...
  # login already warmed the closure via `nix flake archive`). Defaults to a filtered copy of the
  # repo root (../ from this file) — enough for `kit → accountPlan` (root *.nix + greeter/); the
  # heavy, never-imported trees (conformance/examples/docs/VCS) are excluded to bound the closure.
  src ? builtins.path {
    path = ./..;
...
let
  ...
  exprFile = pkgs.writeText "contract-account-plan.nix" ''
    ...
    in
    kit.internal.accountPlan { inherit identity grants; }
  '';
in
pkgs.writeShellApplication {
  ...
  text = ''
    ...
    grants=''${2:-}
    [ -n "$identity" ] && [ -n "$grants" ] || {
      echo "usage: contract-account-plan <identity.json> <grants.json>" >&2
      exit 1
    }
