...
[ADR-0015](../../docs/adr/0015-greeter-secret-provisioning.md)). It is an **example, not contract
code** — the contract ships only the `keyFetcher` *seam* (ADR-0004); this is one way to fill it.
...
- **Trusted Tier-1 seat only.** The seat decrypts the key into the home; an exposed/agent host must
  never run this (the greeter refuses it — ADR-0001/ADR-0015).
