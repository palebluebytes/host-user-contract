...
ADR numbers below point at the current set ([`docs/adr/`](../adr/README.md)), which was rewritten
as one ordered narrative on 2026-08-20. Numbers cited in the originating issue, and in any commit
message predating that rewrite, refer to the older set and will not resolve — read the current index
by subject instead.
...
| `user-inkpotmonkey` (**new**) | The user's home config, ADR-0010 shape. | Created in stage 1. |
...
## Stage 1 — create the user repo (ADR-0010 shape)
...
- `identity.json` — pure data (`name`, `email`, `username`). Loaded by the *binding*, never
  by the home (ADR-0005); the home reads `config.identity.{name,email}` for its dotfiles.
- `home.nix` — a **contract-parameterized home-manager module**: it declares which SESSION SHAPES
  it can run in (`contract.supports.*` — no default, at least one required, ADR-0007), what it asks
  ...
  `hostFacts` (ADR-0010: the grant is derived from the offer).
- `flake.nix` — inputs `contract` + `home-manager` (with `nixpkgs.follows` so there is ONE
  nixpkgs, ADR-0002); `checks = packages`, which gives the home-manager build the contract's own
  ...
  `mkContractUsers` for a multi-user repo) — the pre-built path (ADR-0011/0014). The offer is
  ...
  and narrowed per system by `contract.lib.mkHomeMatrix` (ADR-0014 renamed these from `variants`;
...
- **`exposed`** — `custom.host.exposed = true`, grants `containers` only (was `workstation` before
  ADR-0008 split that role; the external check flips when it next updates its `contract` pin). Proves criterion 6: the
...
**user's own** home sops, decrypted by the user's own age key ([ADR-0004](../adr/0004-user-is-self-contained.md),
...
- [x] **1.** User lives in a separate repo of the ADR-0010 shape, consumed via `bindContractPackage`. *(stages 1, 3; the `inkpotmonkey-home` repo + fleet input)*
...
- [x] **5.** `hashedPassword` handling matches the visibility decision (public → yescrypt in `identity.json`); rationale recorded. *(stage 2; [ADR-0004](../adr/0004-user-is-self-contained.md), user repo `identity-yescrypt` check)*
...
- `mkContractUser` / `mkContractUsers` / `mkContractFleet` / `bindContractUser` / `traceUser` — the public producer/consumer surface (`lib.nix`, ADR-0014). The three producers are one user / a member set you enumerate / one you derive across systems.
