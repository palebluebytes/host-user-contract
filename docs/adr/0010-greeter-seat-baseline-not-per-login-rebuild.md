...
**Status:** Accepted; refines [ADR-0006](0006-anyhost-greeter-runtime-binding.md) (the runtime binding flow) and builds on [ADR-0008](0008-greeter-is-a-contract-deliverable.md) (the greeter is a contract deliverable).
...
**Session launch (ADR-0006 step 8):** the greeter **selects** the session type from the bound home's `gui.session` — contract *mechanism*, conformance-checkable. The **host binds the session backend** per type (`custom.greeter.session.{wayland,x11}`, null-default, like `homeBuilder` and the display backend), and a user's home may override with its own compositor (ADR-0006: packages are the user's self-contained concern). Host-as-primary so a minimal, contract-pure home still gets a session. The contract ships no compositor (package-free, [ADR-0004](0004-extract-contract-flake.md)).
...
- A greeter seat must **declare the baseline** (both session stacks + the bound backend) to host any runtime user; a headless host declares none — incapacity, not a ban ([ADR-0002](0002-user-confinement-manifest-greeter.md)).
