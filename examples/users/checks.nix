...
# and has no second home for the mode-substitution proof to compare.
{
  ...
  lib,
  pkgs,
  # The contract flake itself, for the two surfaces reached here by name: `mkMemberChecks` (the
  # kit) and `mkIdentityPostureCheck` (which the offender probe calls a second time, deliberately,
  # to watch it refuse), plus `floorMode` — the session shape the proofs compare against without
  # naming a literal.
  contract,
...
// {
  ...
  # teaching extra a real users repo does not carry: a posture check that passes because its
  # identity list is empty reads identically to one that passes on merit.
  #
  # So: take the REAL derived members, append one synthetic `$6$` offender, and require that the
  # check rejects it. This tests the CALL SITE, not the helper.
  identity-posture-rejects-an-offender =
    let
      # Any real members identity will do as the base — the offender differs from it only in the
      # two fields the posture looks at — so it is taken from the derivation rather than by naming
      # a user this check has no other business knowing.
      #
      # The offending hash is sha512crypt (`$6$`), which is exactly what the public-repo posture
      # exists to exclude (ADR-0004). So this probes the posture THIS fleet requires, not postures
      # in general: hand `require = "libc"` in and the offender stops offending, and the assert
      # below fires — correctly, and saying so.
    ...
    assert lib.assertMsg rejected (
      ...
      + "NOT fail `require = \"${require}\"`. The posture check above is therefore vacuous — it "
  ...
  # a teaching extra — it is the one check here that names users, because the arrangement it
  ...
  #                   the check RUNS it out of each home-path;
  ...
  #                   the other's.
  #
  # This is also why the check lives with the fleet rather than in `conformance/`: it needs both
  ...
  #
  # Members-generic on purpose, unlike `shared-code-per-user-data` above: WHICH user substitutes
  ...
  # off one home's worth of content, deliberately and correctly (ADR-0027). What this check refuses
  ...
  #
  # WHY THE VERDICT IS IN THE BUILD rather than in an eval `assert`. Divergence is only answerable
  # from realized content, and both cheap eval-time approximations lie:
  #
  ...
  #
  # So the eval `assert` claims only what is knowable before anything is built: that there is a
  # pair to compare at all.
  #
  # THE CONTRACT'S OWN CONTENT IS SUBTRACTED. `.contract-desktop` is composed by `mkContractHome`
  # out of the mode's `desktop` parameter, so counting it would let this check pass on a string the
