...
| [0026](0026-consumer-producer-public-surface.md) | The public surface is the coin `mkContractUser`/`mkContractUsers`/`bindContractUser` + `traceUser`; retires inline-eval and unilateral direct-grant, internalizes the package kernels (amends 0007/0008/0016/0025); amended repeatedly as the surface grew by the front door — most recently 2026-08-17 to fifteen, adding `mkContractFleet` while keeping `mkContractUsers` as the reachable escape hatch; amended 2026-08-19 by [0032](0032-grants-ride-the-bind-modes-build-homes.md), which **deletes `lib.hostFactsFor`** and takes the count back to fourteen |
...
| [0030](0030-one-name-per-value-on-the-producer-surface.md) | One name per value on the producer surface — **one word, one type**: `grants` is the attrset, `grantKey` the sorted list, `granted` only an option path; the producer coin reads its system off `pkgs`; one resolver answers "which user is this?" under one rule; and (amendment) the nouns follow — `variant` → **bake** (one shape, filled progressively), `roster` → **members** / `mkMembers` / `mkMemberChecks`, `userDir` → `memberDir`. Renames only: no behaviour changes, and the manifest wire format was untouched *by this ADR*; amended 2026-08-18 with two corrections to its own record — the "only one exception" claim it contradicts, and the account of `variant`'s deletion — and 2026-08-19 to record that its scoped wire-key exception **fired**: ADR-0032's v3 deleted the `granted` wire key rather than moving it |
| [0031](0031-published-bake-set-unclassified-unpruned.md) | The published bake set is the home matrix, **unclassified and unpruned** — a *negative result*: the contract cannot derive which home content is graphical (no nixpkgs `meta` field; `.desktop` inspection is IFD by string context; the namespace heuristic is [0002](0002-user-confinement-manifest-greeter.md)'s model C and measures zero discrimination), and pruning bakes that coincide is feasible but pays the evaluation it prunes to recover 0.013%; amended 2026-08-19 — the classification half still binds, but per-mode baking ([0032](0032-grants-ride-the-bind-modes-build-homes.md)) makes identical bakes unproducible, so the pruning half's measurement is moot and the mechanism the body reasons over is deleted |
| [0032](0032-grants-ride-the-bind-modes-build-homes.md) | **Grants ride the bind; modes build homes** — `needsOwnHome` fused two facts about `gui` (a host-side grant, and home content), so it is split: every grant now rides the bind and can never change a home, and the session **mode** (`cli`/`gui`, mutually exclusive) is what a home is built for. Deletes the powerset, `homeAxes`, the grant-less bake, the narrowed `hostFacts.granted` and the grants↔home pairing; a user declares `supports.<mode>` (no default, ≥1) and a host's modes are derived from its affordances |

## Numbering

The ADRs are numbered **contiguously, `0001`–`0032`** — the numbering used throughout the
