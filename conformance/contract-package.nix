...
# manifest read with `granted` absent), and an equivalence check that each repo fixture's JSON is
...
let
  ...
  # `referenceFields` is v1 (no `granted`, exercising the v1->v2 compat); `guiFields` is v2.
  referenceFields = {
    version = 1;
    username = "ada";
    requests = {
  ...
  # v1 compat: a v1 manifest (written with `granted` absent) reads back with `granted = [ ]`.
...
in
{
  assertions = [
    ...
    # manifest v1->v2 compat: a v1 manifest (granted absent) reads back with granted = []
    {
      name = "manifest compat: a v1 manifest reads back with granted = [] (v1->v2 compat)";
      ok =
        referenceRoundTrip.version == 1
        && referenceRoundTrip.granted == [ ]
        && builtins.removeAttrs referenceRoundTrip [ "granted" ] == referenceFields;
    }
